...
let cfg = config.pinpkgs.crawl4ai; in {
  options.pinpkgs.crawl4ai.externalDns = {
    enable = lib.mkEnableOption "public external DNS records for crawl4ai";

    domain = lib.mkOption {
      type        = lib.types.str;
      default     = "crawl4ai.example.com";
      description = "Public FQDN for crawl4ai. Provider resolves this to the egress IP or tunnel.";
    };
