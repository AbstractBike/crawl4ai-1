...
let cfg = config.pinpkgs.crawl4ai; in {
  options.pinpkgs.crawl4ai.health = {
    liveness = {
  ...
  config = lib.mkIf cfg.enable {
    pinpkgs.crawl4ai.health.liveness.port  = lib.mkDefault cfg.ports.health;
    pinpkgs.crawl4ai.health.readiness.port = lib.mkDefault cfg.ports.health;
