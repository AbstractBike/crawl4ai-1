...
let cfg = config.pinpkgs.crawl4ai; in {
  options.pinpkgs.crawl4ai.logs = {
    format       = lib.mkOption { type = lib.types.enum [ "json" "text" ]; default = "json"; };
    journaldUnit = lib.mkOption { type = lib.types.str; default = "crawl4ai.service"; description = "systemd unit to collect logs from"; };
    filePaths    = lib.mkOption { type = lib.types.listOf lib.types.str; default = []; description = "Additional log file paths (if not journald)"; };
    fields       = lib.mkOption { type = lib.types.attrsOf lib.types.str; default = { service = "crawl4ai"; }; description = "Extra fields added to every log record"; };
  };
