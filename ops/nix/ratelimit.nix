...
let cfg = config.pinpkgs.crawl4ai; in {
  options.pinpkgs.crawl4ai.ratelimit = {
  ...
  config = lib.mkIf (cfg.enable && cfg.ratelimit.enable) {
    services.nginx.virtualHosts.${cfg.routing.lanDomain}.extraConfig = ''
      limit_req_zone $binary_remote_addr zone=crawl4ai:10m rate=${toString cfg.ratelimit.requestsPerSecond}r/s;
      limit_req zone=crawl4ai burst=${toString cfg.ratelimit.burstSize} nodelay;
