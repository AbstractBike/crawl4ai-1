...
let cfg = config.pinpkgs.crawl4ai; in {
  options.pinpkgs.crawl4ai.temporal = {
    ...
    taskQueue  = lib.mkOption { type = lib.types.str; default = "crawl4ai"; };
  ...
  config = lib.mkIf (cfg.enable && cfg.temporal.worker.enable) {
    systemd.services.crawl4ai-worker = {
      description = "crawl4ai Temporal worker";
      after       = [ "network-online.target" "crawl4ai.service" ];
      ...
      serviceConfig = {
        User       = cfg.user;
        ExecStart  = "${if cfg.temporal.worker.package != null then cfg.temporal.worker.package else cfg.package}/bin/crawl4ai-worker";
        Restart    = "always";
        RestartSec = "5s";
      };
    };
